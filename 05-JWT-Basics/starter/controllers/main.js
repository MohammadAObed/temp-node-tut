...
const login = async (req,res) => {
    ...
    if(!username || !password) {
        throw new CustomAPIError('Please provide email and password') //we are throwing bcz of the package of express-async-errors (it says in the docs to throw errors), if we don't use the package, we would create the async wrapper middleware and use next here instead of throw.
    }
    ...
    //1-(payload) no confidential info in it (ex: password), try to keep payload small, better for (ux)!!!. If we creating a user then the id is useful to pass in payload, a user checking his resources that belongs to them (so thats why id of user is good)
...
//a-front end requesting the dashboard route, reqeust header contains the token, and token is obtained from the local storage
...
//Payload: place the info: like the id of user that just logged on or registered (request): then we send back the enitre token with that payload back to the front end, then the frontend sends it back to us (in another front-end restricted request), and then when we decode we get that id
